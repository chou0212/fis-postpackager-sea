...
module.exports = function (ret, conf, settings, opt) { //打包后处理
    ...
    function seajsCodeGen(map) {
        ...
        fis.util.map(map.res, function (id, file){
            var uri = file.uri;
            if (file.pkg){
                uri = map.pkg[file.pkg].uri;
            }
            if (!ret.ids[id].isCssLike && !/sea-config/.test(id)) {
                fis_sea_conf.alias[id] = fis_sea_conf.alias[id] || uri;
            }
        });
        
        return 'seajs.config(' + JSON.stringify(fis_sea_conf, null, opt.optimize ? null : 4) + ');';
    }
    ...
    function createConfigFile(asyncList, subpath) {
        var code = settings.codeGen(asyncList);
        var cfg = fis.file(fis.project.getProjectPath(), subpath);
        cfg.setContent(code);
        fis.log.notice(ret);
        ret.pkg[cfg.subpath] = cfg;
        ret.ids[cfg.getId()] = cfg;
        ret.map.res[cfg.getId()] = {
            uri: cfg.getUrl(opt.hash, opt.domain),
            type: "js"
        };
        return cfg;
    }
    ...
    //var isInjected = 0;
    /**
     * 注入seajs配置文件，将会注入map.json资源表
     * @param content
     * @returns {XML|string|void}
     */
    function injectSeaJsConfig(content) {
        //if (isInjected) return;
        var subpath = 'cfg/sea-config.js';
        //构造map.js配置文件
        var file = createConfigFile(ret.map, subpath);
        //isInjected = 1;
    ...
    function injectAutoLoad(file, include) {
        ...
        if (settings.type === 'seajs') {
            if (/\bseajs\.use\s*\(/.test(content)) {
                content = injectSeaJsConfig(content);
            }
        } else {
